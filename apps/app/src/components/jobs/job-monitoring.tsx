  ...
  return (
    <div className={`${bgColor} p-3 rounded`}>
      ...
      <div className="text-xs space-y-1">
        ...
        {job.processedOn && (
          <p>Processing: {new Date(job.processedOn).toLocaleString()}</p>
        )}
        {job.failedReason && (
          <p className="text-red-600">Error: {job.failedReason}</p>
        )}
      </div>
      {job.data && (
