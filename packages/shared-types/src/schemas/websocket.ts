...
export const webSocketEventSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('WORKFLOW_RUN_POLLING'), data: workflowRunInfoSchema }),
  z.object({ type: z.literal('WORKFLOW_RUN_CANCELLED'), data: workflowRunInfoSchema }),
  z.object({ type: z.literal('WORKFLOW_RUN_DELETED'), data: workflowRunInfoSchema }),
  z.object({ type: z.literal('SOURCE_QUERY_STARTED'), data: pluginRunSchema }),
  z.object({ type: z.literal('SOURCE_QUERY_COMPLETED'), data: pluginRunSchema }),
  z.object({ type: z.literal('SOURCE_QUERY_FAILED'), data: pluginRunSchema }),
  z.object({ type: z.literal('PIPELINE_EXECUTION_STARTED'), data: z.object({ workflowRunId: z.string(), sourceItemId: z.string() }) }),
  z.object({ type: z.literal('PIPELINE_EXECUTION_COMPLETED'), data: z.object({ workflowRunId: z.string(), sourceItemId: z.string() }) }),
  z.object({ type: z.literal('PIPELINE_EXECUTION_FAILED'), data: z.object({ workflowRunId: z.string(), sourceItemId: z.string(), error: z.string().optional() }) }),
  ...
  z.object({ type: z.literal('JOB_REMOVED'), data: z.object({ jobId: z.string(), queueName: z.string() }) }),
  z.object({ type: z.literal('QUEUE_STATUS_UPDATED'), data: z.array(queueStatusSchema) }),
]);
